...
color_light_purple = "#DD6FF8"
color_white = "#EAE1E1"
color_light_blue = "#7BB9E6"
color_pink = "#E23697"
color_black = "black"

#window setup
window = tkinter.Tk() #create the window
window.title("Mai'sCalculator")
...
for row in range(row_count):
    for column in range(column_count):
        ...
        if value in top_symbols:
            button.config(foreground=color_black, background=color_light_purple)
        elif value in right_symbols:
            button.config(foreground=color_white, background=color_pink)
        else:
            button.config(foreground=color_white, background=color_light_blue)
